...
module('01 - Templates Acceptance Tests when no scriptwriters', function(hooks) {
  ...
  skip("01 - Templates - 01 - Should display comics", async function (assert) {
    ...
    let $comicItems = findAll('.comics-list > .comics-list-item');
...
module('01 - Templates Acceptance Tests when empty', function(hooks) {
  ...
  skip("01 - Templates - 04 - Should display message if empty", async function (assert) {
    ...
    assert.equal(findAll('.comics-list > .comics-list-item').length, 0, "No item found");

    assert.ok(find('.comics-list').textContent.indexOf("Sorry, no comic found") >= 0, "Empty message displayed");
  });
});
